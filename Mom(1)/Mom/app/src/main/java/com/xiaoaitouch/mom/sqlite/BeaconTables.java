...
public class BeaconTables {
	...
	public static final String CREATE_TABLE = "CREATE TABLE "
			...
			+ mac + " TEXT ," + name + " TEXT ," + low_battery + " TEXT ,"
			+ createdate + " TEXT ," + last_connect_date + " TEXT ," + desc
			+ " TEXT ," + last_broken_time + " TEXT ," + power + " INTEGER"
			+ ");";
	...
	public static ArrayList<BeaconInfo> getBeaconList(Context context) {
		...
		while (cursor.moveToNext()) {
			...
			info.setUuid(cursor.getString(cursor
					.getColumnIndex(BeaconTables.uuid)));
			info.setName(cursor.getString(cursor
					.getColumnIndex(BeaconTables.name)));
			info.setDesc(cursor.getString(cursor
					.getColumnIndex(BeaconTables.desc)));
			arrayList.add(info);
		}
		cursor.close();
	...
	public static boolean addBeacons(Context context, BeaconInfo info) {
		for (BeaconInfo bInfo : getBeaconList(context)) {
			if (bInfo.getUuid().equals(info.getUuid())) {
				return false;
			}
		}
		DBHelper dbHelper = new DBHelper(context);
		String[] arrayKey = { uuid , name
				, low_battery, createdate
				, last_connect_date, desc
				, last_broken_time };
		String[] arrayValues = { info.getUuid(), info.getName()
				, info.getLow_battery(), info.getCreatedate()
				, info.getLast_connect_date(), info.getDesc()
				, info.getLast_broken_time() };
	 ...
	 * @return
	 */
	public static boolean delBeacons(Context context, String mac) {
		DBHelper dbHelper = new DBHelper(context);
		return dbHelper.delete(TABLE_NAME, uuid, mac);
	}

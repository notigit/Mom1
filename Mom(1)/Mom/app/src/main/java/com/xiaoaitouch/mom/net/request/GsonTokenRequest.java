...
public abstract class GsonTokenRequest<T> extends
		ParamsRequest<JsonResponse<T>> {
	private Gson mGson = new Gson();
	private Listener<JsonResponse<T>> mListener;

	public GsonTokenRequest(String url, Listener<JsonResponse<T>> listener,
			ErrorListener error) {
		this(Method.GET, url, listener, error);
	}

	public GsonTokenRequest(int method, String url,
			Listener<JsonResponse<T>> listener, ErrorListener error) {
	...
	@Override
	protected Response<JsonResponse<T>> parseNetworkResponse(
		...
		try {
			...
			return Response.success(fromJson, HttpHeaderParser
					.parseCacheHeaders(response, getExpireStrategy()));

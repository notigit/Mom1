...
public class DBHelper {
     ...
     *
     * @param table
     * @param arrayKey    字段 数组
     * @param arrayValues 与字段相应的值数组
     * @return
     */
     ...
     * @return
     */
    public boolean update(String table, String[] keys, String[] values, String whereKey, String whereValues, String whereKeyss, String whereValuess) {
        String sql_content = "";
        for (int i = 0; i < keys.length; i++) {
            if (i == (keys.length - 1)) {
                sql_content += keys[i] + "='" + values[i] + "' ";
        ...
        sql_content = sql_content + "WHERE " + whereKey + " = '" + whereValues + "'" + " AND " + whereKeyss + " = '" + whereValuess + "'";
        String sql = "UPDATE " + table + " SET " + sql_content + ";";
    ...
    public boolean update(String table, String[] keys, String[] values, String[] whereKey, String[] whereValues) {
        ...
        for (int i = 0; i < whereKey.length; i++) {
            if (i == 0) {
                sql_content = sql_content + "WHERE " + whereKey[0] + " = '" + whereValues[0] + "'";
            } else {
                sql_where = sql_where + " AND " + whereKey[i] + " = '" + whereValues[i] + "'";
            }
        }
        sql_statement = sql_content + sql_where;
        String sql = "UPDATE " + table + " SET " + sql_statement + ";";
        SQLiteDatabase db = openHelper.getWritableDatabase();
        try {
            db.execSQL(sql);
            db.setTransactionSuccessful();
